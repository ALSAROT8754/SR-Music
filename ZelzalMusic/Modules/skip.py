...
from ZelzalMusic import BOT_USERNAME, app, zelzaldb, pytgcalls
from ZelzalMusic.Helpers import _clear_, admin_check, buttons, close_key, gen_thumb
...
async def skip_str(_, message: Message):
    try:
        ...
        pass
    get = zelzaldb.get(message.chat.id)
    if not get:
        try:
            await _clear_(message.chat.id)
            await pytgcalls.leave_group_call(message.chat.id)
            await message.reply_text(
                text=f"- الـتـالـي 🥺\n \n- بواسطة : {message.from_user.mention} 🥀\n\n**- لايوجـد اغـاني** {message.chat.title}, **🕷**",
